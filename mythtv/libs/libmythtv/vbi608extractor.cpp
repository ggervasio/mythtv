...
static float find_clock_diff(const QList<float> &list)
{
    ...
    {
        LOG(VB_VBI, LOG_DEBUG, QString("max_diff too big:  %1").arg(max_diff/avg_diff, 4, 'f', 2));
        return 0.0f;
    }
    if (avg_diff * 0.70 > min_diff)
    {
        LOG(VB_VBI, LOG_DEBUG, QString("min_diff too small:  %1").arg(min_diff/avg_diff, 4, 'f', 2 ));
        return 0.0f;
    }
